...
def fetch_sentences(filename):
    ...
    with open(filename, 'r') as f:
        for line in f:
            sentences_str += line.decode('utf-8')
...
def main():
    ...
    with open('result/unit2_word_freq.csv', 'w') as f:
        ...
        for word in word_usage:
            try:
                f.write('%s,%.4f,%.4f,%.4f\n' % (word['word'].decode('ascii'), word['event'], word['baseline'], word['diff']))
            except:
    ...
    with open('result/unit2_lemma_freq.csv', 'w') as f:
        ...
        for lemma in lemma_usage:
            try:
                f.write('%s,%.4f,%.4f,%.4f\n' % (lemma['word'].decode('ascii'), lemma['event'], lemma['baseline'], lemma['diff']))
            except:
                print "Could not decode"
    lemma_diff = get_lemma_diff(lemma_usage, key='diff')
    with open('result/unit2_final_result.csv', 'w') as f:
        f.write("word, diff\n")
        for lemma in lemma_diff:
            try:
                f.write('%s,%.4f\n' % (lemma['word'].decode('ascii'), lemma['diff']))
            except:
                print "Could not decode"
    end = timeit.default_timer()
