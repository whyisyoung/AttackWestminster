...
val warcPath = "/user/cs4984cs5984f18_team4/4_Hurricane_Sandy/Hurricane_Sandy.warc.gz" //be aware on DLRL cluster, this will be HDFS path
val cdxPath = "/user/cs4984cs5984f18_team4/4_Hurricane_Sandy/Hurricane_Sandy.cdx" //be aware on DLRL cluster, this will be HDFS path
...
// cleaned_df.repartition(1).write.mode("overwrite").format("json").save("/user/cs4984cs5984f18_team4/4_Hurricane_Sandy/sentences") // export data to your local path;be aware on DLRL cluster, this will be HDFS path
...
val result_df = result.toDF("originalUrl", "timestamp", "title", "text").filter($"title" !== "").filter($"title" !== "Error Page").filter($"title" !== "403 -Forbidden") // convert to DataFrame format in order to export Json format and perform SQL queries as needed

result_df.repartition(1).write.mode("overwrite").format("json").save("/user/cs4984cs5984f18_team4/4_Hurricane_Sandy/jusText_sentences") // export data to your local path;be aware on DLRL cluster, this will be HDFS path
